  ...
  },
  {
    classMethods: {
      ...
      async addIgnoreDuplicateRule() {
        try {
          await Model.query(`CREATE RULE "clip_on_duplicate_ignore" AS ON INSERT TO ${this
            .tableName}
      ...
      async search(userId, searchPhrase, resultLimit) {
        ...
        const vectorName = this.getVectorName();
        return Model.query(
          `SELECT id, title, author, text, slug FROM "${this.tableName}"
